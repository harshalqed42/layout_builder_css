...
namespace Drupal\layout_builder_css\EventSubscriber;
...
class BlockRendererListener implements EventSubscriberInterface
{
    ...
    {
        drupal_set_message('enters here first');
        ...
        if ($access->isAllowed()) {
            ...
            $css_id = $event->getComponent()->get('layout_builder_css_id');
            if ($css_id) {
                //@todo : To make the library name dynamically
                $build['#attached']['library'][] = 'layout_builder_css/css/' . $css_id;
            }
